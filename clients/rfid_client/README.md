...
An Arduino/PlatformIO client that implements the **RFID step** in the RFID → keypad → doorlock flow. The RFID client reads RFID tag UIDs from an MFRC522 reader over **I2C**, then publishes scans to an **MQTT broker** as JSON. It also publishes an **online status** message when it connects.
